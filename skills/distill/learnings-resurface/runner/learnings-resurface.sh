...
# ---------- config ----------
VAULT="${VAULT_PATH:-$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Vault}"
...
# Auto-discover the active project memory directory.
# Override with VAULT_MEMORY_DIR env var if needed.
memory_dir = os.environ.get("VAULT_MEMORY_DIR")
if not memory_dir:
    candidates = sorted(
        glob.glob(os.path.expanduser("~/.claude/projects/*/memory")),
        key=lambda p: os.path.getmtime(os.path.join(p, "MEMORY.md"))
            if os.path.exists(os.path.join(p, "MEMORY.md")) else 0,
        reverse=True,
    )
    memory_dir = candidates[0] if candidates else os.path.expanduser("~/.claude/projects/default/memory")
...
run_phase_b() {
  ...
  if [[ "$kept" -eq 0 ]]; then
    ...
    echo '{"run_id": "'"$RUN_ID"'", "phase": "B", "clusters_routed": 0, "clusters_queued": 0, "router_promotions": [], "playbook_distills": [], "learnings_appends": [], "tasks_created": [], "contested": []}' > "$routed_file"
    return 0
  fi
...
Output: execute the route + log workflow per SKILL.md section B. Apply 5-cap. Write each cluster to its proposed destinations (memory file, MEMORY.md index line, playbook distill, _learnings.md append, optional task-manager-MCP task — only what type+rules require). For commitment clusters, only invoke a task-creation MCP tool if proposed_destinations.task_destination is non-null AND the corresponding mcp_tool is in your allowed-tools — otherwise skip MCP and write to _learnings.md Key Decisions only. Append per-cluster + roll-up lines to Vault/log.md. Write Phase B's routed.json to $RUN_DIR/routed.json per the schema in SKILL.md section B.6. End your turn after writing routed.json."
...
idx.setdefault("runs", {})[run_id] = {
    ...
    "tasks_created": routed.get("tasks_created", []),
