...
#
# Iterates over Reading/Articles, Reading/Books, Reading/Tweets in your Obsidian vault.
...
#
# Setup: edit VAULT, CLI_BIN, CLI_FLAG below to match your machine.
# CLI_BIN can point at any single-shot/headless coding agent (Claude Code,
# Codex CLI, etc). For non-Claude agents you may also need to update the
# prompt body in process_file() — see comments there.

set -euo pipefail

# ---------- config (set via env vars or edit defaults) ----------
# VAULT_PATH env var takes precedence; falls back to common Obsidian locations.
if [[ -n "${VAULT_PATH:-}" ]]; then
    VAULT="$VAULT_PATH"
elif [[ -d "$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Vault" ]]; then
    VAULT="$HOME/Library/Mobile Documents/iCloud~md~obsidian/Documents/Vault"
elif [[ -d "$HOME/Documents/Vault" ]]; then
    VAULT="$HOME/Documents/Vault"
else
    echo "ERROR: vault not found. Set VAULT_PATH env var or edit this script." >&2
    exit 1
fi

# CLI agent binary. Auto-discovered from PATH; override with CLAUDE_BIN env var.
# Works for any headless agent with a single-shot prompt flag (Claude Code: -p, Codex CLI: similar).
if [[ -n "${CLAUDE_BIN:-}" ]]; then
    CLI_BIN="$CLAUDE_BIN"
else
    CLI_BIN="$(command -v claude 2>/dev/null || echo "$HOME/.local/bin/claude")"
fi
CLI_FLAG="${CLI_FLAG:--p}"                # single-shot/headless flag (claude uses -p)
...
# Verify CLI binary
[[ -x "$CLI_BIN" ]] || { echo "CLI agent not found at $CLI_BIN" >&2; exit 1; }
...
process_file() {
  ...
  # For Claude Code (skill installed): the "Use the auto-playbook-distill skill"
  # line activates the skill. For Codex CLI / other agents: replace that line
  # with "Follow the workflow in <repo>/prompts/auto-playbook-distill.md" or
  # paste the prompt body inline.
...
Use the auto-playbook-distill skill to process this file."
  ...
  # Invoke the CLI agent in single-shot/headless mode.
  local out
  if out=$("$CLI_BIN" "$CLI_FLAG" "$prompt" 2>&1); then
